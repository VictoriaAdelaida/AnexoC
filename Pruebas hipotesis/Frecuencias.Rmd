...
# Extract total_frecuencia values for each group
...
# Create vectors of counts and sample sizes
...
if (sum(x_counts) > 0 && all(x_counts <= n_counts, na.rm = TRUE)) {
  for (g in 1:4) {
    ...
    if (n_group > 0 && n_other > 0 && x_group <= n_group && x_other <= n_other) {
      ...
      # If significant, add the group to the results
